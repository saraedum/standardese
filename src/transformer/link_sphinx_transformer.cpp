...
link_sphinx_transformer::link_sphinx_transformer(model::unordered_entities& documents, struct options options, inventory::sphinx::documentation_set inventory) : transformer(documents), options(std::move(options)), inventory(std::move(inventory)), target_transformer(documents, inventory::symbols(this->inventory)) {}

void link_sphinx_transformer::transform(threading::pool::factory workers) {
  target_transformer.transform(workers);
  transformer::transform(workers);
}

void link_sphinx_transformer::do_transform(model::entity& document) {
