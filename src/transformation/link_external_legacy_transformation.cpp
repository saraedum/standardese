    ...
    if constexpr (std::is_same_v<T, model::markup::link>) {
      link.target.accept([&](auto&& target) {
        ...
        if constexpr (std::is_same_v<T, model::link_target::standardese_target>) {
          ...
          std::smatch match;
          if (std::regex_match(target.target, match, util::regex::link_external_legacy_transformation_pattern) && match.str(2) == options.namspace) {
            link.target = model::link_target::uri_target(std::regex_replace(options.url, util::regex::link_external_legacy_transformation_replace, match.str(1)));
          }
        }
      });
    }
