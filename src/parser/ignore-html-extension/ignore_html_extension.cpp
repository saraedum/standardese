...
namespace standardese::parser::ignore_html_extension
{
...
{
    using unique_iter = std::unique_ptr<cmark_iter, std::integral_constant<decltype(cmark_iter_free)&, cmark_iter_free>>;
    ...
    while (true)
    {
        switch (cmark_iter_next(iter.get()))
        {
            ...
            case CMARK_EVENT_ENTER:
                cmark_iter_reset(iter.get(), postprocess(cmark_iter_get_node(iter.get())), CMARK_EVENT_ENTER);
                break;
        }
    }
}
