...
TEST_CASE("Headings are Generated for C++ Entities", "[entity_heading_transformer]") {
  ...
  SECTION("Heading for a Header File") {
    ...
    SECTION("When the Format String is Empty, No Heading is Generated") {
      entity_heading_transformer{parsed.entities, {""}}.transform();
    ...
    SECTION("The Format String must Describe a Heading") {
      REQUIRE_THROWS(entity_heading_transformer{parsed.entities, {"not a heading"}}.transform());
    }

    SECTION("The Format String can Contain the Header Name") {
      entity_heading_transformer{parsed.entities, {"# {{ name }}"}}.transform();
    ...
    SECTION("If the Format String Contains the Brief, it is Omitted Later") {
      entity_heading_transformer{parsed.entities, {R"(# {{ name }} — {{ md(section("brief")) }}{{ drop_section("brief") }})"}}.transform();
